...
const FractionalKnapsackDemo: React.FC = () => {
  const [step, setStep] = useState<number>(0);
  const [approach, setApproach] = useState<'greedy' | 'random'>('greedy');
  const [capacity, setCapacity] = useState<number>(50);
  const [cargo, setCargo] = useState<CargoItem[]>([]);
  const [showSimulation, setShowSimulation] = useState<boolean>(false);

  const [newCargo, setNewCargo] = useState<NewCargoForm>({
    name: '',
    weight: '',
    profit: '',
  });
  ...
  const calculateGreedySolution = (): StepData[] => {
    const cargoWithRatio = cargo
      .map(item => ({
        ...item,
    ...
    for (let i = 0; i < cargoWithRatio.length; i++) {
      ...
      if (remaining >= item.weight) {
        // Take full item
        const fullItem: CargoItem = {
          ...item,
          amount: item.weight,
          fractional: false,
        };
        loaded = [...loaded, fullItem];
        remaining -= item.weight;
        totalProfit += item.profit;

        steps.push({
          title: `Step ${stepNum}: Add ${item.name}`,
          desc: `Ratio: ${(item.ratio ?? 0).toFixed(1)} | Adding ${item.weight} tons`,
          loaded: [...loaded],
          remaining,
          totalProfit,
        });
        ...
        loaded = [...loaded, partialItem];
        totalProfit += fractionalProfit;

        steps.push({
          title: `Step ${stepNum}: Partial ${item.name}`,
          desc: `Only ${remaining.toFixed(
            1,
          )} tons left! Take ${(fraction * 100).toFixed(1)}% of ${item.name}`,
          loaded: [...loaded],
          remaining: 0,
          totalProfit,
        });
  ...
  const calculateRandomSolution = (): StepData[] => {
    const steps: StepData[] = [
      {
        title: 'Initial Setup',
  ...
  const greedySteps: StepData[] =
    cargo.length > 0 ? calculateGreedySolution() : [];
  const randomSteps: StepData[] =
    cargo.length > 0 ? calculateRandomSolution() : [];

  const currentSteps: StepData[] =
    approach === 'greedy' ? greedySteps : randomSteps;
  ...
  const currentStep = currentSteps[step] ?? emptyStep;

  const nextStep = () => {
    if (step < currentSteps.length - 1) setStep(prev => prev + 1);
  };
  ...
  const switchApproach = (newApproach: 'greedy' | 'random') => {
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600">
              Configure your ship and cargo details
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              ...
              <input
                ...
                onChange={e =>
                  setNewCargo({ ...newCargo, weight: e.target.value })
                }
                className="px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-blue-500 focus:outline-none"
              ...
              <input
                ...
                onChange={e =>
                  setNewCargo({ ...newCargo, profit: e.target.value })
                }
                className="px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-blue-500 focus:outline-none"
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b-2 border-gray-200">
                      ...
                      <th className="text-center p-3 text-gray-700">
                        Action
                      </th>
                    </tr>
                  </thead>
          ...
          {cargo.length > 0 ? (
            ...
            <div className="bg-yellow-50 border-2 border-yellow-200 rounded-xl p-6 text-center">
              ...
              <p className="text-gray-700">
                Add cargo items or load sample data to begin
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
          ...
          <div className="bg-gradient-to-r from-blue-100 to-blue-50 rounded-lg p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-sm text-gray-600">
                {(capacity - currentStep.remaining).toFixed(1)}/
                {capacity} tons loaded
              </div>
            </div>
            ...
            <div className="w-full bg-gray-200 rounded-full h-8 mb-4 overflow-hidden">
              <div
                ...
                style={{
                  width: `${
                    ((capacity - currentStep.remaining) / capacity) *
                    100
                  }%`,
                }}
              >
            ...
            <div className="space-y-2">
              {currentStep.loaded.length === 0 ? (
                ...
                currentStep.loaded.map((item, idx) => (
                  <div
                    key={idx}
                    className={`${item.color} bg-opacity-20 border-l-4 ${item.color.replace(
                      'bg-',
                      'border-',
                    )} p-3 rounded`}
                  >
        ...
        {step === currentSteps.length - 1 && (
          <>
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl shadow-lg p-6 border-2 border-green-200 mb-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="text-3xl font-bold text-orange-600">
                    ₹
                    {Number(
                      randomSteps[randomSteps.length - 1]?.totalProfit.toFixed(
                        1,
                      ),
                    )}
                    L
                ...
                <div>
                  ...
                  <p className="text-3xl font-bold text-green-600">
                    ₹
                    {Number(
                      greedySteps[greedySteps.length - 1]?.totalProfit.toFixed(
                        1,
                      ),
                    )}
                    L
              ...
              <div className="mt-4 pt-4 border-t border-green-200">
                <div className="flex items-center justify-between">
                  ...
                  {randomSteps.length > 0 &&
                  randomSteps[randomSteps.length - 1].totalProfit > 0 ? (
                    <span className="text-2xl font-bold text-green-600">
                      +
                      ₹
                      {(
                        greedySteps[greedySteps.length - 1].totalProfit -
                        randomSteps[randomSteps.length - 1].totalProfit
                      ).toFixed(1)}
                      L (
                      {(
                        ((greedySteps[greedySteps.length - 1].totalProfit -
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
              ...
              <div>
                <h4 className="text-lg font-semibold text-gray-700 mb-4">
                  Profit Growth Over Steps
                </h4>
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart
                    data={greedySteps.map((s, idx) => ({
                      step: idx,
                      'Greedy (Dayal)': Number(
                        s.totalProfit.toFixed(1),
                      ),
                      'Random (Rajnikanth)': Number(
                        (
                          randomSteps[idx]?.totalProfit ??
                          randomSteps[randomSteps.length - 1]
                            ?.totalProfit ??
                          0
                        ).toFixed(1),
                      ),
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mt-6">
          ...
          {approach === 'greedy' ? (
            <div className="text-gray-700 space-y-2">
              <p>
                • <strong>Time Complexity:</strong> O(n log n) - dominated
                by sorting
              </p>
              <p>
                • <strong>Strategy:</strong> Sort by profit-to-weight ratio,
                pick highest first
              </p>
              <p>
                • <strong>Key Advantage:</strong> Guarantees optimal solution
            ...
            <div className="text-gray-700 space-y-2">
              <p>
                • <strong>Strategy:</strong> Load items without calculation
                or planning
              </p>
              <p>
                • <strong>Result:</strong> Suboptimal profit due to poor item
                selection
              </p>
              <p>
                • <strong>Loss:</strong> Missing potential profit compared to
                greedy approach
              </p>
            </div>
          )}
